...
from motion_predictor import TrackEncoder
        ...
        while batch is not None:
            with acc.accumulate(model):
                ...
                pred = model(
                    ...
                    future_tracks=batch["future_tracks"]
                )
                loss = {}
...
if __name__ == "__main__":
    ...
    init_pg_kwargs = InitProcessGroupKwargs(timeout=timedelta(seconds=36000))
    ...
    train_start_ratio = cfg.get("train_start_ratio") or 0.0
    train_end_ratio = cfg.get("train_end_ratio") or 1.0
    track_dataset_kwargs = {k: cfg.get(k) for k in ["num_tracks", "track_length", "track_dimensionality"] if cfg.get(k) is not None}
    train_dataset = LMDBdst_jpeg(
        ...
        cfg["action_mode"],
        cfg["act_dim"],
        start_ratio=train_start_ratio,
        end_ratio=train_end_ratio,
        **track_dataset_kwargs,
    )
    test_dataset = LMDBdst_jpeg(
        cfg["LMDB_validation_path"], 
        cfg["seq_len"], 
        cfg["chunk_size"], 
        cfg["action_mode"],
        cfg["act_dim"],
    ...
    if cfg.get("use_future_tracks"):
        track_encoder = TrackEncoder(track_length=cfg["track_length"], **cfg["track_encoder_kwargs"])
    else:
        track_encoder = None
    model = GR1(
        model_clip,
        ...
        use_future_tracks=cfg.get("use_future_tracks") if cfg.get("use_future_tracks") is not None else False,
