...
class GR1(nn.Module):
    def __init__(
        ...
        use_hand_rgb=True,
        use_future_tracks=False,
        track_encoder=None,
        track_drop_prob=None,
        track_constant_interval=1,
        **kwargs
    ):
        super().__init__()
        ...
        self.use_future_tracks = use_future_tracks
        if use_future_tracks:
            assert track_encoder is not None
            self.track_encoder = track_encoder
            self.track_encoder_output_to_features = nn.Linear(track_encoder.hidden_size, hidden_size)
            self.track_drop_prob = track_drop_prob
            if track_drop_prob is not None:
                self.track_drop_replace_embedding = torch.nn.Parameter(hidden_size)
            self.track_constant_interval = track_constant_interval
    ...
    def forward(
        ...
        if self.use_future_tracks:
            future_track_embeddings, _ = self.track_encoder(future_tracks.flatten(0, 1))
            future_track_embeddings = future_track_embeddings.view(batch_size, sequence_length, -1)
        ...
        if self.use_future_tracks:
            future_track_embeddings = self.track_encoder_output_to_features(future_track_embeddings)
        ...
        if self.use_future_tracks:
            future_track_embeddings = future_track_embeddings + time_embeddings.unsqueeze(0)
        ...
        if self.use_future_tracks:
            future_track_embeddings = future_track_embeddings.view(batch_size, sequence_length, 1, self.hidden_size)
            if self.track_drop_prob is not None:
                to_drop = torch.rand(batch_size) < self.track_drop_prob
                future_track_embeddings[to_drop, :, :, :] = self.track_drop_replace_embedding[None, None, None, :]
            stacked_inputs = torch.cat((stacked_inputs, future_track_embeddings), dim=2)
        if self.act_pred:
        ...
        attention_mask_repeat = n_lang_tokens + n_state_tokens + n_patch_tokens + n_obs_tokens
        if self.use_hand_rgb:
            attention_mask_repeat = attention_mask_repeat + n_hand_patch_tokens + n_hand_obs_tokens
        if self.use_future_tracks:
            attention_mask_repeat = attention_mask_repeat + n_future_track_tokens
            
        stacked_attention_mask = stacked_attention_mask.repeat(1, 1, attention_mask_repeat)
