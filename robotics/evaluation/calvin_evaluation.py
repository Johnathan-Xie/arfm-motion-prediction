...
class GR1CalvinEvaluation(CalvinBaseModel):
    def __init__(
        ...
        motion_predictor_frame_initial_resize_height=256,
        motion_predictor_frame_initial_resize_width=256,
        motion_predictor_sampling_kwargs={
            "num_timesteps_to_sample": 16,
            "num_steps": 16,
            "update_num_steps": 4,
            "update_noising_start_timestep": 0.8
        },
        reset_point_prediction_on_new_goal=False,
        track_horizon_prediction_interval=1,
    ):
        """Constructor."""
        ...
        self.policy = policy
        self.track_horizon_prediction_interval = track_horizon_prediction_interval

        self.motion_predictor_pipeline = motion_predictor_pipeline
        if motion_predictor_pipeline is not None:
            self.motion_predictor_frame_initial_resize_height = motion_predictor_frame_initial_resize_height
            self.motion_predictor_frame_initial_resize_width = motion_predictor_frame_initial_resize_width
            self.motion_predictor_sampling_kwargs = motion_predictor_sampling_kwargs
            self.current_text_goal = None
            self.reset_point_prediction_on_new_goal = reset_point_prediction_on_new_goal
            self.predicted_tracks_list = []
    ...
    def reset(self):
        ...
        self.current_text_goal = None
        if self.motion_predictor_pipeline is not None:
            self.motion_predictor_pipeline.reset_caches()
    ...
    def step(self, obs, goal):
        ...
        # Motion Prediction
        if self.motion_predictor_pipeline is not None:
            input_frame = torch.from_numpy(obs["rgb_obs"]["rgb_static"]).unsqueeze(0)
            input_frame = media.resize_video(input_frame, (self.motion_predictor_frame_initial_resize_height, self.motion_predictor_frame_initial_resize_width))
            if self.current_text_goal is None or (goal != self.current_text_goal and self.reset_point_prediction_on_new_goal): 
                self.motion_predictor_pipeline.reset_caches()
                predicted_tracks = self.motion_predictor_pipeline.initialize_online_future_point_prediction(
                    starting_frames=[input_frame],
                    text_inputs=[goal],
                    sampling_kwargs=self.motion_predictor_sampling_kwargs,
                )
                self.current_text_goal = goal
            else:
                #if (goal != self.current_text_goal and self.reset_point_prediction_on_new_goal):
                #    update_sampling_kwargs = deepcopy(self.motion_predictor_sampling_kwargs)
                #    update_sampling_kwargs["update_num_steps"] = update_sampling_kwargs["num_steps"]
                #    update_sampling_kwargs["update_noising_start_timestep"] = 0
                #else:
                #    update_sampling_kwargs = self.motion_predictor_sampling_kwargs
                predicted_tracks = self.motion_predictor_pipeline.update_online_future_point_prediction(
                    new_frames=[input_frame],
                    text_inputs=[goal],
                    sampling_kwargs=self.motion_predictor_sampling_kwargs,
                    predict_horizon=self.rollout_step_counter % self.track_horizon_prediction_interval == 0,
                )
            predicted_tracks = predicted_tracks.squeeze(0)
        else:
        ...
        if predicted_tracks is not None:
            self.predicted_tracks_list.append(predicted_tracks)
        ...
        if predicted_tracks is not None:
            predicted_tracks_data = torch.zeros((1, self.seq_len, ) + predicted_tracks.shape)
            predicted_tracks_tensor = torch.stack(self.predicted_tracks_list, dim=0)
            predicted_tracks_data[0, :buffer_len] = predicted_tracks_tensor
        else:
            predicted_tracks_data = None
        # State
        ...
        if predicted_tracks is not None:
            predicted_tracks_data = predicted_tracks_data.to(self.device)
        ...
        with torch.no_grad():
            prediction = self.policy(
                ...
                future_tracks=predicted_tracks_data,
            )
        ...
        if predicted_tracks is not None:
            output["predicted_tracks"] = predicted_tracks.cpu().numpy()
        return output
