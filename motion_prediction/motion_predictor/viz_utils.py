...
def paint_point_track(
    ...
    """
    if visibles is None:
        visibles = np.ones_like(point_tracks[..., 0])
...
def get_horizon_colormap(horizon: int) -> List[Tuple[int, int, int]]:
    """Generates aesthetic gradient: Blue -> Cyan -> Green -> Yellow -> Red"""
    cmap = []
    for h in range(horizon):
        ratio = h / max(horizon - 1, 1)
...
def paint_future_point_track(
    frames: np.ndarray,
    point_tracks: np.ndarray
) -> np.ndarray:
    """Converts a sequence of points to color coded video based on timestep."""

    num_frames, num_points, horizon = point_tracks.shape[:3]
    timestep_colormap = get_horizon_colormap(horizon)

    height, width = frames.shape[1:3]
    dot_size_as_fraction_of_min_edge = 0.01
    radius = int(round(min(height, width) * dot_size_as_fraction_of_min_edge))
    diam = radius * 2 + 1
    quadratic_y = np.square(np.arange(diam)[:, np.newaxis] - radius - 1)
    quadratic_x = np.square(np.arange(diam)[np.newaxis, :] - radius - 1)
    ...
    for t in range(num_frames):
        ...
        for i in range(num_points):
            for h in range(horizon):
                ...
                # Color based on timestep h (aesthetic gradient)
                color = np.array(timestep_colormap[h])[np.newaxis, np.newaxis, :]
                image[y1:y_ub, x1:x_ub, :] = (1 - patch) * image[
                    y1:y_ub, x1:x_ub, :
                ] + patch * color
